...
function signOutUnauthorizedUser() {
  firebase.auth().signOut();
  showAuthMessage("This account is not approved for access.");
}
...
function initAuth() {
  try {
    ...
    showAuthMessage("Firebase config missing. Add your Firebase keys in script.js.");
  ...
  function formatAuthError(err) {
    ...
    if (code === "auth/unauthorized-domain") {
      return "This domain is not authorized in Firebase Auth. Add this site domain in Firebase Console -> Authentication -> Settings -> Authorized domains.";
    }
    ...
    if (code === "auth/popup-blocked" || code === "auth/popup-closed-by-user") {
      return "Popup sign-in was blocked/closed. Retrying with redirect sign-in...";
    }
    return `Sign-in failed (${code}).`;
  }

  firebase.auth().setPersistence(firebase.auth.Auth.Persistence.LOCAL).catch(() => {
    showAuthMessage("Could not persist login session in this browser.");
  });
  ...
  firebase.auth().getRedirectResult().then((result) => {
    if (result && result.user) {
      showAuthMessage(`Signed in as ${(result.user.email || "").toLowerCase()}. Loading app...`);
    }
  }).catch((err) => {
  ...
  firebase.auth().onAuthStateChanged((user) => {
    if (!user) {
      ...
      if (!authMessage.textContent) {
        showAuthMessage("Please sign in with Google.");
      }
      return;
    }
    ...
    showAuthMessage(`Signed in as ${email}. Loading app...`);
    enableApp();
  });
}
